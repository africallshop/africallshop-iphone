...
- (void)setAfricallLog:(AfriCallShopCalLog *)acallLog {
    africallLog = acallLog;
    [self update];
}
#pragma mark - Action Functions
    ...
    if(africallLog != NULL) {
        ...
        if(controller != nil) {
           // [controller setCallLogId: [NSString stringWithUTF8String:linphone_call_log_get_call_id(callLog)]];
            [controller setCallshopLog:africallLog];
        }
    }
}
...
//	}
    ...
    image = [UIImage imageNamed:@"call_status_outgoing.png"];
    if (![africallLog.nom_contact isEqualToString:@""]) {
        address = africallLog.nom_contact;
    }
    else
    {
        address=africallLog.to;
    }

...
//- (void)setCallLogId:(AfriCallShopCalLog *)log {
//    //self->callLogId = [acallLogId copy];
//    callshopLog=log;
//    [self update];
//}
-(void) setCallshopLog:(AfriCallShopCalLog *)log
{
    callshopLog=log;
    [self update];
}
...
//    }
    ...
    // Set Address
    if([callshopLog.nom_contact length]) {
        //address = NSLocalizedString(@"Unknown", nil);
        address = callshopLog.nom_contact;
    }
    else
    {
        address = callshopLog.to;
    }
...
//    }
   ...
   // NSDate *startData = [NSDate dateWithTimeIntervalSince1970:linphone_call_log_get_start_date(callLog)];
    ...
    [dateLabel setText:[dateFormatter stringFromDate:callshopLog.callDate]];
...
//    [durationLabel setText:[NSString stringWithFormat:@"%02i:%02i", (duration/60), duration - 60 * (duration / 60), nil]];
      ...
      [durationLabel setText:callshopLog.callDuration];
    
    // contact name
    [plainAddressLabel setText:callshopLog.calledCountry];
//    if (addr != NULL) {
...
//    ABRecordRef contactV = [[[LinphoneManager instance] fastAddressBook] getContact:normalizedSipAddress];
//    
//    if(contactV) {
//        displayName = [FastAddressBook getContactDisplayName:contactV];
//    }
//    if (addr != NULL) {
//        char* lAddress = linphone_address_as_string_uri_only(addr);
//        if(lAddress != NULL) {
            [ContactSelection setAddAddress:address];

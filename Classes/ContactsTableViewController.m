...
-(void)loadAfricallShopContact
{
    ...
    NSURL *url=[NSURL URLWithString:@"https://www.africallshop.com/api/get_contacts_ios.php"];
    ...
    if ([response statusCode] >=200 && [response statusCode] <300)
    {
        ...
        for ( NSDictionary* dic in jsonData)
        {
            ...
            if ([dic objectForKey:@"nom"]!= (NSString *)[NSNull null])
            {
                name=[dic objectForKey:@"nom"];
                ABRecordSetValue(contact, kABPersonFirstNameProperty, name,nil);
            }
            if ([dic objectForKey:@"numero_portable"]!= (NSString *)[NSNull null])
            {
                ABMutableMultiValueRef multiPhone = ABMultiValueCreateMutable(kABMultiStringPropertyType);
                ABMultiValueAddValueAndLabel(multiPhone,[dic objectForKey:@"numero_portable"]  , kABPersonPhoneMobileLabel, NULL);

...
typedef enum _ViewElement {
    ...
    ViewElement_Email2 = 105,
    ViewElement_PhoneNumber = 106,
...
- (IBAction)onSignInClick:(id)sender {
    ...
    UserAccount *afriUser= [[UserAccount alloc] init];
    afriUser.email=username;
    afriUser.firsPasswd=password;
    NSMutableString *errors = [NSMutableString string];
    if ([username length] == 0) {
    ...
    if([errors length]) {
        ...
        //Get authentified here
        NSString *post =[[NSString alloc] initWithFormat:@"login=%@&password=%@",username,password];
        NSLog(@"PostData: %@",post);
        
        NSURL *url=[NSURL URLWithString:@"https://www.africallshop.com/api/auth_account_ios.php"];
        ...
        if ([response statusCode] >=200 && [response statusCode] <300)
        {
            ...
            // NSString *temp=[responseData substringWithRange:NSMakeRange(16, 178)];
            //  NSLog(@"%@",temp);
            ...
            {
                NSLog(@"Login SUCCESS");
                [self alertStatus:NSLocalizedString(@"Logged in Successfully.",nil) :NSLocalizedString(@"Login Success!",nil)];
                afriUser.username=[jsonData objectForKey:@"Username"];
                afriUser.password=[jsonData objectForKey:@"Password"];
                afriUser.domain=[jsonData objectForKey:@"Domain"];
                afriUser.server=[jsonData objectForKey:@"Server"];
                afriUser.balance=[jsonData objectForKey:@"Balance"];
                afriUser.callerId=[jsonData objectForKey:@"CallerId"];
                afriUser.dislayName=[jsonData objectForKey:@"DisplayName"];
                
                [appDelegate setAfriCallUser:afriUser];
                NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
                
                [defaults setObject:appDelegate.afriCallUser.email forKey:@"email"];
                [defaults setObject:appDelegate.afriCallUser.firsPasswd forKey:@"firsPasswd"];
                [defaults setObject:appDelegate.afriCallUser.username forKey:@"username"];
                [defaults setObject:appDelegate.afriCallUser.password forKey:@"password"];
                [defaults setObject:appDelegate.afriCallUser.dislayName forKey:@"DisplayName"];
                [defaults setObject:appDelegate.afriCallUser.server forKey:@"server"];
                [defaults setObject:appDelegate.afriCallUser.domain forKey:@"domain"];
                [defaults setObject:appDelegate.afriCallUser.callerId forKey:@"callerId"];
                
                [defaults synchronize];
                
                [self addProxyConfig:afriUser.username password:afriUser.password domain:afriUser.domain server:nil];
...
//                NSString *error_msg = (NSString *) [jsonData objectForKey:@"error_message"];
                [self alertStatus:NSLocalizedString(@"Please check your password or username", nil) :NSLocalizedString(@"Login Failed!",nil)];
            ...
            [self alertStatus:NSLocalizedString(@"Connection Failed",nil) :NSLocalizedString(@"Login Failed!",nil)];
        }
...
- (IBAction)onRegisterClick:(id)sender {
    ...
    NSString *email2 = [WizardViewController findTextField:ViewElement_Email2 view:contentView].text;
    NSString *phoneNumber = [WizardViewController findTextField:ViewElement_PhoneNumber view:contentView].text;
    ...
    if (![email2 isEqualToString:email]) {
        [errors appendString:NSLocalizedString(@"The emails are different.\n", nil)];
    }
    NSPredicate *phoneNumberTest = [NSPredicate predicateWithFormat:@"SELF BEGINSWITH[cd] %@", @"00"];
    if(![phoneNumberTest evaluateWithObject:phoneNumber]) {
        [errors appendString:NSLocalizedString(@"The phone number is invalid.\n", nil)];
    }
    if ([phoneNumber length]< 10) {
        [errors appendString:NSLocalizedString(@"The phone number should have at least 10 digits.\n", nil)];
    }
    ...
    if([errors length]) {
        ...
        NSString *post =[[NSString alloc] initWithFormat:@"nom=%@&prenom=%@&adresseMail=%@&adresseMailConfirmation=%@&motDePasse=%@&motDePasseConfirmation=%@&numeroDeTelephone=%@",username,@"test",email,email2,password,password2,phoneNumber];
        NSLog(@"PostData: %@",post);
        
        NSURL *url=[NSURL URLWithString:@"https://www.africallshop.com/api/register_ios.php"];
        ...
        if ([response statusCode] >=200 && [response statusCode] <300)
        {
            ...
            NSDictionary *jsonData = (NSDictionary *) [jsonParser objectWithString:responseData error:nil];
            NSLog(@"%@",jsonData);
            if ([[jsonData objectForKey:@"CompteExiste"] isEqualToString:@"KO"]) {
                UIAlertView* errorView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Check error(s)",nil)
                                                                    message:NSLocalizedString(@"This account already exists",nil)
                                                                   delegate:nil
                                                          cancelButtonTitle:NSLocalizedString(@"Continue",nil)
                                                          otherButtonTitles:nil,nil];
                [errorView show];
            ...
            }else
            {
                UIAlertView* infoView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Information",nil)
                                                                   message:NSLocalizedString(@"Your account have been registered successfully",nil)
                                                                  delegate:nil
                                                         cancelButtonTitle:NSLocalizedString(@"Continue",nil)
                                                         otherButtonTitles:nil,nil];
                [infoView show];

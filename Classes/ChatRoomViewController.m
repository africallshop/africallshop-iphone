...
//    }
    ABRecordRef contactV = [[[LinphoneManager instance] fastAddressBook] getContact:remoteAddress];
...
//    }
  ...
  //  [chat create];
   ...
   // [chat release];
    ...
    NSString *post =[[NSString alloc] initWithFormat:@"login=%@&password=%@&numero=%@&message=%@",email,firsPasswd,remoteAddress,message];
    NSLog(@"PostData: %@",post);
    
    NSURL *url=[NSURL URLWithString:@"https://www.africallshop.com/api/sms_ios.php"];
    ...
    if ([response statusCode] >=200 && [response statusCode] <300)
    {
        ...
        //0 IDLE, 1 in progress, 2 delivered, 3 not delivered see LinphoneChatMessageState
        NSLog(@"Response ==> %@", responseData);
        if ([responseData isEqualToString:@"OK"]) {
            chat.state=[NSNumber numberWithInt:2];
            [chat update];
        ...
        {
            chat.state=[NSNumber numberWithInt:3];
            [chat update];
            [tableController updateChatEntry:chat];
            NSLog(@"SMS not sent");
        }
